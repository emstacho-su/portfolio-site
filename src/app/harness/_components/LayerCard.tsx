...
export function LayerCard({ layer }: LayerCardProps) {
  return (
    ...
    >
      <span
        className="font-mono text-sm text-tertiary self-start pt-1 tabular-nums"
        aria-hidden="true"
      >
      ...
      <div className="flex flex-col gap-2">
        ...
        <ul className="flex flex-wrap gap-1.5 mt-1" aria-label={`${layer.name} tooling`}>
          {layer.badges.map((badge) => (
            <li
              key={badge}
              className="font-mono text-[10px] tracking-wide px-2 py-1 border border-hairline text-tertiary"
            >
              {badge}
            </li>
          ))}
